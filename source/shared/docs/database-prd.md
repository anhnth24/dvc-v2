...
**Quick Reference:** [Database Summary](../../quick-reference/database-summary.md)
**Code Examples:** [Database Examples](../../code-examples/database/)

---

## 1. Executive Summary

Database infrastructure nội bộ hỗ trợ 800,000 hồ sơ/tháng với 21,000 concurrent civil servants qua SQL Server 2022 với simplified architecture. Single primary database với materialized views cho read optimization, regional cache nodes (không full sharding), read replicas cho analytics. Redis Enterprise cache layer với 6-node cluster, MinIO object storage với erasure coding (8+4) cho 200TB→500TB scaling. Performance targets: <20ms database queries, 11-nines data durability, 99.9% availability với automatic failover <30s cho hệ thống cán bộ công chức.

## 2. Scope & Objectives

### 2.1 In Scope
- **SQL Server 2022:** Simplified architecture với materialized views, Always On Availability Groups cho hệ thống nội bộ
...
### 3.1 Simplified Database Architecture (Single Database + Materialized Views)

#### 3.1.1 Philosophy: Smart Simplification

**Challenge:** Full CQRS with separate databases adds 60% operational complexity without proportional benefits for most use cases.

**Solution:** Single database with intelligent read optimization through materialized views and smart caching.

#### 3.1.2 Single Database Configuration

**Unified Database Setup:**
```sql
-- Single database optimized for both reads and writes
-- Materialized views for complex reads
-- Covering indexes for performance
-- Always consistent (no eventual consistency issues)

ALTER DATABASE DVC_MainDB SET
  COMPATIBILITY_LEVEL = 160,
  AUTO_CREATE_STATISTICS ON,
  AUTO_UPDATE_STATISTICS ON,
  DELAYED_DURABILITY = DISABLED, -- Ensure durability
  QUERY_STORE = ON (
    OPERATION_MODE = READ_WRITE,
    CLEANUP_POLICY = (STALE_QUERY_THRESHOLD_DAYS = 30),
    DATA_FLUSH_INTERVAL_SECONDS = 900,
    MAX_STORAGE_SIZE_MB = 2000, -- Larger for unified DB
    INTERVAL_LENGTH_MINUTES = 60
  );
```

**Benefits of Single Database Approach:**
- **60% Operational Complexity Reduction**: Single database to manage
- **Immediate Consistency**: No synchronization delays or conflicts
- **Simplified Backup/Recovery**: Single backup strategy
- **Same Performance**: Materialized views provide read optimization
- **Easier Development**: Single connection string, unified testing

#### 3.1.3 Materialized Views for Read Performance

**Document Dashboard Materialized View:**
```sql
-- Create materialized view for dashboard (refreshed automatically)
...
SELECT
  ...
  CAST(COUNT_BIG(*) AS int) AS AttachmentCount -- Required for indexed view
...
INNER JOIN dbo.Users u ON wi.AssignedUserId = u.Id
LEFT JOIN dbo.Attachments a ON d.Id = a.DocumentId
WHERE d.IsDeleted = 0
GROUP BY d.Id, d.Title, d.SubmissionDate, d.Status,
         p.Name, p.Category, pu.Name, wi.CurrentStep,
         wi.DeadlineDate, u.FullName;

-- Create clustered index on materialized view (makes it truly materialized)
CREATE UNIQUE CLUSTERED INDEX IX_DocumentDashboard_Clustered
ON vw_DocumentDashboard(Id);

-- Create covering indexes for common queries
CREATE NONCLUSTERED INDEX IX_DocumentDashboard_ProcessingUnit_Status
ON vw_DocumentDashboard(ProcessingUnit, Status, UrgencyStatus)
INCLUDE (Title, ProcedureName, AssignedToName, DeadlineDate);
```

**Performance Analytics Materialized View:**
```sql
-- Performance metrics aggregated view
CREATE VIEW vw_PerformanceMetrics WITH SCHEMABINDING AS
SELECT
  pu.Name AS ProcessingUnit,
  p.Category AS ProcedureCategory,
  YEAR(d.SubmissionDate) AS Year,
  MONTH(d.SubmissionDate) AS Month,
  COUNT_BIG(*) AS TotalDocuments,
  COUNT_BIG(CASE WHEN d.Status = 'Completed' THEN 1 END) AS CompletedDocuments,
  AVG(CASE
    WHEN d.CompletionDate IS NOT NULL
    THEN CAST(DATEDIFF(hour, d.SubmissionDate, d.CompletionDate) AS float)
    END) AS AvgProcessingHours,
  AVG(CASE
    WHEN d.CompletionDate IS NOT NULL AND d.CompletionDate <= wi.DeadlineDate
    THEN 1.0 ELSE 0.0
...
INNER JOIN dbo.WorkflowInstances wi ON d.Id = wi.DocumentId
WHERE d.IsDeleted = 0
GROUP BY pu.Name, p.Category, YEAR(d.SubmissionDate), MONTH(d.SubmissionDate);

-- Create clustered index
CREATE UNIQUE CLUSTERED INDEX IX_PerformanceMetrics_Clustered
ON vw_PerformanceMetrics(ProcessingUnit, ProcedureCategory, Year, Month);
```

#### 3.1.4 Smart Read Routing Strategy
...
public class SmartDbContext : DbContext
{
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        ...
        {
            // Use read replicas for specific read-heavy queries
            options.ApplicationIntent = GetApplicationIntent();
        });
    }

    private ApplicationIntent GetApplicationIntent()
    {
        // Check if current operation is read-heavy analytics
        var context = HttpContext.Current;
        if (context?.Request.Path.Value?.Contains("/api/analytics") == true ||
            context?.Request.Path.Value?.Contains("/api/reports") == true)
        {
            return ApplicationIntent.ReadOnly; // Route to read replica
        }

        return ApplicationIntent.ReadWrite; // Use primary for everything else
    }
}

// Repository pattern with smart view selection
public class DocumentRepository : IDocumentRepository
{
    public async Task<PagedResult<DocumentSummary>> GetDocumentDashboardAsync(int userId, DocumentFilter filter)
    {
        // Use materialized view for dashboard queries (50x faster than joins)
        return await _context.vw_DocumentDashboard
            .Where(d => d.ProcessingUnit == GetUserProcessingUnit(userId))
            .Where(filter.BuildExpression())
    ...
    {
        // Use regular tables for detailed operations
...
#### 3.1.5 Performance Comparison: Simplified vs Full CQRS

| Aspect | Full CQRS | Simplified Approach | Benefit |
|--------|-----------|-------------------|---------|
| **Operational Complexity** | High (2 databases to manage) | Low (1 database) | 60% reduction |
| **Read Performance** | Very Fast (denormalized) | Very Fast (materialized views) | Same |
| **Write Performance** | Fast (normalized) | Fast (same structure) | Same |
| **Data Consistency** | Eventual (sync delays) | Immediate (single DB) | Better |
| **Backup Strategy** | Complex (coordinate 2 DBs) | Simple (single backup) | Much simpler |
| **Development Speed** | Slow (manage 2 schemas) | Fast (single schema) | 40% faster |
| **Error Handling** | Complex (sync failures) | Simple (ACID transactions) | Much simpler |
